...
# TODO delete between(pjs) '# # # #' the mathod was Implemented in Sampler
...
class Sampler:
    ...
    def __init__(self, graph_part, block_id):
        ...
        # self.crosslayer = connect(self.graph_part, self.crosslayer_dis) # TODO(pjs) delete
    ...
    def renewp(self, newp):
        """An interface for updating the value of table in Sampler"""
        self.p_table = newp
    ...
    def get_crosslayer(self):
        ...
        for i in range(len(self.graph_part)):
            cl.append(self.BreadthFirstSearch(i))
        return cl

    def BreadthFirstSearch(self, node_id):
        res_list = []
        q = Queue()
        q.put([node_id, 0])
        v = []
