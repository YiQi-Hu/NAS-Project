...
def dis_global(enu,eva,num_gpu):
	...
	for gpu in range(0,num_gpu):     #not test first_samper
		gpu_list.put(gpu)
		Net_item.cell ,Net_item.graph, Net_item.code =nn.spl.sample();tmp=copy.deepcopy(Net_item)
	...
	while i<200:
		...
		for result in results[i-2:]:
			...
			print(eva_result,i-2+k,Net.item_list[i-2+k].score,Net.item_list[i-2+k].cell,Net.item_list[i-2+k].graph,flush=True);
